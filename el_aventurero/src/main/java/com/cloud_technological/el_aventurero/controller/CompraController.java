...
public class CompraController {
    ...
    @PostMapping("/create")
    public ResponseEntity<ApiResponse<CompraDto>> create(@Valid @RequestBody CreateCompraDto createDto) {
        try {
            CompraDto result = compraService.create(createDto);
            ApiResponse<CompraDto> response = new ApiResponse<>(
    ...
    public ResponseEntity<ApiResponse<Object>> delete(@PathVariable("id") Long id) {
        try {
            Boolean isDeleted = compraService.delete(id);
    ...
    public ResponseEntity<ApiResponse<CompraDto>> findById(@PathVariable Long id) {
        try {
            CompraDto result = compraService.findById(id);
            ApiResponse<CompraDto> response = new ApiResponse<>(
                HttpStatus.OK.value(),
                "Elemento encontrado",
                false,
    ...
    public ResponseEntity<ApiResponse<Object>> page(@Valid @RequestBody PageableDto<Object> pageableDto) {
        try {
            Page<CompraTableDto> result = compraService.pageCompras(pageableDto);
            if (result.isEmpty()) {
