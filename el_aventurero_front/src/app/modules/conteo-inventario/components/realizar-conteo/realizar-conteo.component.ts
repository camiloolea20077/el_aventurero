...
export class RealizarConteoComponent implements OnInit {
  ...
  ngOnChanges() {
    if (this.displayModal) {
      ...
      // ✅ NUEVO: Si hay conteoId, cargar ese conteo
      if (this.conteoId) {
        this.cargarConteoExistente();
      } else {
        this.resetear();
      }
    }
  ...
  agregarProducto(): void {
    ...
    if (yaAgregado) {
      this._alertService.showError(
        'Error',
        'Este producto ya fue agregado al conteo',
      );
      return;
    }

    // Obtener stock del sistema (desde el inventario del producto)
    const inventarioProducto = this.inventarios.find(
      (inv) => inv.producto_id === this.productoSeleccionado!.id,
    );
  ...
  async completarConteo(): Promise<void> {
    ...
    const diferenciasSinAjustar = this.productosConteo.filter(
      (pc) => pc.diferencia !== 0 && !pc.ajustado,
    );

    if (diferenciasSinAjustar.length > 0) {
      this._alertService.showError(
        'Diferencias Detectadas',
        `Hay ${diferenciasSinAjustar.length} producto(s) con diferencias. Debes justificar todas las diferencias antes de completar el conteo.`,
      );
      return;
    }
  ...
  async recargarDetalles(): Promise<void> {
    ...
    try {
      const response = await lastValueFrom(
        this.conteoInventarioService.getDetallesByConteoId(
          this.conteoActual.id,
        ),
      );
  ...
  getStockSistema(productoId: number): number {
    const inventario = this.inventarios.find(
      (inv) => inv.producto_id === productoId,
    );
  ...
  async cargarConteoExistente(): Promise<void> {
    ...
    try {
      ...
      this.conteoActual = conteoResponse.data;
      ...
      // Convertir detalles a ProductoConteo
      this.productosConteo = [];
      ...
      // Ir directamente al paso de conteo
      this.paso = 'CONTEO';

      this.messageService.add({
        severity: 'info',
        summary: 'Conteo Cargado',
        detail: 'Continuando con el conteo anterior',
        life: 3000,
      });
    } catch (error) {
